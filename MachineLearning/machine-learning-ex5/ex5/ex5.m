...
lambda = 3;
[theta] = trainLinearReg(X_poly, y, lambda);
error_val = linearRegCostFunction(X_poly_test, ytest, theta, 0);
error_val
...
%%randomly
    ...
    sel_X_poly = X_poly(sel(1:i),:);
    sel_y = y(sel(1:i));
    sel = randperm(size(X_poly_val, 1));
    sel_X_poly_val = X_poly_val(sel(1:i),:);
    sel_yval = yval(sel(1:i));
    [theta] = trainLinearReg(sel_X_poly, sel_y, lambda);
    error_train(i) = error_train(i)  + linearRegCostFunction(sel_X_poly, sel_y, theta, 0);
    error_val(i)  = error_val(i)  + linearRegCostFunction(sel_X_poly_val, sel_yval, theta, 0);
    end
end
error_train = error_train /iter;
error_val = error_val /iter;
